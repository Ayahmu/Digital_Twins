<template>
  <div class="container-p">
    ...
    <div class="picture" ref="picture"></div>
  </div>
</template>
<script>
...
let pressure = connectdata[3];
let density = connectdata[4];
let humidity = connectdata[5];
let flow = connectdata[6];
let energy = connectdata[7];
let water = connectdata[8];
export default {
  data() {
    return {
      pressure,
      ...
      boxes: [
        ...
        { isActive: false, content: "制氢能耗数据" },
        { isActive: false, content: "制氢水耗数据" },
      ],
      receivedMessage: 1,
    };
  },
  ...
  methods: {
    ...
    init(shunxu) {
      ...
      var data1 = [pressure, density, humidity, flow, energy, water];
      var img = [
      ...
      var data = [
        {
          coords: XName.map(function (value, index) {
            return [value, data1[shunxu][index]];
          }),
        },
      ];
