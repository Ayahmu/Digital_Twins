  ...
  methods: {
    ...
    update() {
      this.formattedAlarms = this.alarm.map((item) => [
        `<span style='color: #00ffff;font-size:18px;'>${item.datetime}</span>`,
        `<span style='color: #00ffff;font-size:18px;'>${item.name}</span>`,
        `<span style='color: #ff0000;font-size:18px;'>${item.diagnosis}</span>`,
        `<span style='color: #ffc700;font-size:18px;'>${item.optimization}</span>`,
      ]);
      this.status = {
        waitTime: 4000,
        headerBGC: "#3472bb",
